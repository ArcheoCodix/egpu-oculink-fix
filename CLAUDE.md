...
|-------|---------|----------|------------|-------|
| **A** — GPU idle wake (DS_GFXCLK) | SCLK 0–60 MHz + Load 0–1% → ring submit → 60s timeout. Rings: `gfx_0.0.0` (kwin flip gap=2, CS2 VKRenderThread gap=3), `comp_1.0.1` (VKD3D, gap=1), `sdma1`+`gfx` (DXVK). Affects native Vulkan (CS2) and DXVK/VKD3D. DS_GFXCLK enters in <2 s after load drops. | simple ring reset (no MES, no MODE1) | `KWIN_DRM_NO_DIRECT_SCANOUT=1` — **ineffective** (kwin still offender via composition path, not just direct scanout). No working workaround. | **Opportunistic only.** GPU must reach deep idle. Not forceable. |
